namespace NServiceBus.RavenDB
{
    ...
    using RavenSessionFactory = NServiceBus.RavenDB.Persistence.RavenSessionFactory;
    using RavenUnitOfWork = NServiceBus.RavenDB.Persistence.RavenUnitOfWork;
    using RavenUserInstaller = NServiceBus.RavenDB.Persistence.RavenUserInstaller;
    using StoreAccessor = NServiceBus.RavenDB.Persistence.StoreAccessor;
    ...
    public static class ConfigureRavenPersistence
    {
        internal static void ThrowIfStoreNotConfigured(this Configure config)
        {
            ...
            {
                throw new Exception("Call config.RavenDBStorage() first.");
            }
        }
        ...
        public static Configure RavenDBStorage(this Configure config)
        {
        ...
        /// <returns>The configuration object.</returns>
        public static Configure RavenDBStorage(this Configure config, string connectionStringName)
        {
        ...
        /// <returns>The configuration object.</returns>
        public static Configure RavenDBStorage(this Configure config, string connectionStringName, string database)
        {
        ...
        /// <returns>The configuration object.</returns>
        public static Configure RavenDBStorage(this Configure config, Func<string> getConnectionString)
        {
        ...
        /// <param name="database">The database name to use.</param>
        /// <returns>The configuration object.</returns>
        public static Configure RavenDBStorage(this Configure config, Func<string> getConnectionString, string database)
        {
            var connectionString = GetRavenConnectionString(getConnectionString);
            return RavenPersistenceWithConnectionString(config, connectionString, database);
        }
        ...
        /// <returns>The configuration object.</returns>
        public static Configure CustomiseRavenDBStorage(this Configure config, Action<IDocumentStore> callback)
        {
        ...
        static void VerifyConnectionToRavenDBServer(IDocumentStore store)
        {
            ...
            try
            {
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    {
                        var jsonMessageSerializer = new JsonMessageSerializer(null);
                        ravenBuildInfo = jsonMessageSerializer.DeserializeObject<RavenBuildInfo>(reader.ReadToEnd());
                    }
