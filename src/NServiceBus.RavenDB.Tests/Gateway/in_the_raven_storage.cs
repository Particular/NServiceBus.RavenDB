namespace NServiceBus.Gateway.Tests.Idempotency.Raven
{
    ...
    using GatewayMessage = NServiceBus.RavenDB.Gateway.Persistence.GatewayMessage;
    ...
    public class in_the_raven_storage
    {
        ...
        public void SetUp()
        {
            ...
            //store = new DocumentStore { Url = "http://localhost:8081" };
            store.Conventions.DefaultQueryingConsistency = ConsistencyOptions.AlwaysWaitForNonStaleResultsAsOfLastWrite;
            
            store.Initialize();

            ravenPersister = new RavenPersistence(new StoreAccessor(store));
        }
        ...
        protected TestMessage GetStoredMessage(string clientId)
        {
            using (var session = store.OpenSession())
            {
                ...
                var messageStored = session.Load<GatewayMessage>(RavenPersistence.EscapeClientId(clientId));
