            ...
            }.Initialize())
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    for (var i = 0; i < 10000; i++)
                    {
                        ...
                        await persister.Add(td, options);
                ...
                while (!finishedAdding || startSlice < lastTimeout)
                {
                    ...
                    foreach (var timeoutData in timeoutDatas.DueTimeouts)
                    {
                        ...
                        var data = await persister.Remove(timeoutData.Id, options);
                        Assert.NotNull(data);
                        found++;
                    }
                }
                ...
                while (true)
                {
                    ...
                    {
                        var data = await persister.Remove(tuple.Id, options);
                        Assert.NotNull(data);
                    }
            ...
            }.Initialize())
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    }.Initialize())
                    {
                        ...
                        for (var i = 0; i < insertsPerThread; i++)
                        {
                            ...
                            await persister2.Add(td, options);
                            Interlocked.Increment(ref expected);
                            lastExpectedTimeout = (td.Time > lastExpectedTimeout) ? td.Time : lastExpectedTimeout;
                        }
                    }
                ...
                while (!finishedAdding1 || !finishedAdding2 || startSlice < lastExpectedTimeout)
                {
                    ...
                    foreach (var timeoutData in timeoutDatas.DueTimeouts)
                    {
                        ...
                        var data = persister.Remove(timeoutData.Id, options);
                        Assert.NotNull(data);
                        found++;
                    }
                }
